...
echo '<tr>';
echo '<td>CODIGO PRODUCTO</td><td>DESCRIPCION</td><td>FECHA MOVIMIENTO</td><td>CANTIDAD</td><td>OBSERVACIONES</td><td>FACTURA COMPRA</td><td>DOCUMENTO </td>';
echo '</tr>';
if ($filas_movimientos  > 0 )
{ 
 while($mov = mysql_fetch_assoc($consulta_movimientos))
 	{
	   $sql_traer_numero_factura ="select numero_factura from $tabla11 where id_factura = '".$mov['id_factura_venta']."' ";
